...
- **2026-03-01** — Bug fix: `parse-resume` was calling `MODELS.synthesis` (Opus) × 3 in parallel. On Vercel Hobby plan `maxDuration` is silently capped at 10s — Opus calls take 30-60s → Vercel kills the function → Anthropic SDK throws `Connection error.`. Fix: switch all three extractions in `parse-resume` to `MODELS.generation` (Sonnet), which completes in 3-8s. Also fixed `export const maxDuration` placement (was between imports; moved after all imports in both routes). To restore Opus quality, upgrade to Vercel Pro and switch back to `MODELS.synthesis` in `parse-resume`.
- **2026-03-01** — Export feature + OpenClaw docs: (1) Added "Download .txt" button next to "Copy" in both the Generate tab (done state) and Pipeline tab (expanded doc viewer) — uses zero-dependency `Blob` + `URL.createObjectURL`, filenames like `company-role-resume.txt`; module-level `downloadTxt` + `toSlug` helpers; (2) Created `openclaw/SETUP.md` with full step-by-step setup guide (Telegram bot, API key, gateway, pairing, verification, keep-alive, cost tips).
