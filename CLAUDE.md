...
- **2026-03-01** — Bug fix: added `export const maxDuration = 90` to `app/api/parse-resume/route.ts` and `app/api/generate/route.ts`; `export const maxDuration = 60` to `app/api/score-opportunity/route.ts`. All three routes called Claude (Opus/Sonnet) but had no timeout config — Vercel's 10s default was causing "Failed to parse resume" errors on step 01 of onboarding.
